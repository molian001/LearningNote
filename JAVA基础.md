...
##2.抽象类的意义
为其提供一个公共的类型，封装子类中的重复内容，子类中虽有不同的实现，但定义是一致的

##3.内部类总结
1.内部类可以随意使用外部类的成员变量（包括私有）而不用生成外部类的对象

2.静态内部类：静态内部类是不需要依赖于外部类的，这点和类的静态成员属性有点类似，并且它不能使用外部类的非static成员变量或者方法

3.私有内部类：只希望被外部类的方法操作

4.方法内部类：只能在方法体内访问

5.匿名内部类：匿名内部类是唯一一种没有构造器的类。正因为其没有构造器，所以匿名内部类的使用范围非常有限，大部分匿名内部类用于接口回调。匿名内部类在编译的时候由系统自动起名为Outter$1.class。一般来说，匿名内部类用于继承其他类或是实现接口，并不需要增加额外的方法，只是对继承方法的实现或是重写。
...
##4.父类的静态方法能否被子类重写
不能
所谓静态，就是在运行时，虚拟机已经认定此方法属于哪个类。  专业术语有严格的含义,用语要准确."重写"只能适用于实例方法.不能用于静态方法.对于静态方法,只能隐藏(在子类中定义和父类相同的名字的方法，将父类中静态方法隐藏),重载,继承.	
...
垃圾回收算法：[http://www.cnblogs.com/caca/p/jvm_gc_algorithm_heap_space.html](http://www.cnblogs.com/caca/p/jvm_gc_algorithm_heap_space.html)
...
##9.Java中==和equals的区别，equals和hashCode的区别
...
hashcode是对该对象进行hash算法得到其对应的hash值，比较的时候先必将两个对象hashcode是否相等，不相等则是两个不同的对象，相等再进行equals方法比较。多用于hashmap，hashtable里，大大减少对象之间比较的次数，方便快速查找和快速比较。

**两个不同的对象hashcode可能相同，但是hashcode相同不一定是同一个对象**

默认情况下，java的hashcode方法是对对象的存储地址进行映射，**当我们重写对象的equals方法时 一定要重写该对象的hashcode方法！！！**

##10.HashMap实现原理
HashMap概述：    HashMap是基于哈希表的Map接口的非同步实现。此实现提供所有可选的映射操作，并允许使用null值和null键。此类不保证映射的顺序，特别是它不保证该顺序恒久不变。

HashMap的数据结构： 在java编程语言中，最基本的结构就是两种，一个是数组，另外一个是模拟指针（引用），所有的数据结构都可以用这两个基本结构来构造的，HashMap也不例外。HashMap实际上是一个“链表散列”的数据结构，即**数组和链表的结合体**。
...
与此类似的还有float Float;double Double;long Long等
...
详情：http://www.cnblogs.com/yuanermen/archive/2009/08/05/1539917.html


##18.ArrayMap VS HashMap

**HashMap**

![](http://lvable.com/wp-content/uploads/2015/08/QQ%E6%88%AA%E5%9B%BE20150823235946.png)
...
当我们第一次新建一个HashMap对象的时候，默认的容量是16，若你只打算在HashMap里放入3个元素那将浪费至少13个空间。

**ArrayMap**

![](http://lvable.com/wp-content/uploads/2015/08/QQ%E6%88%AA%E5%9B%BE20150824001700.png)
...
![](http://lvable.com/wp-content/uploads/2015/08/QQ%E6%88%AA%E5%9B%BE20150824002101.png)

这个数据结构的设计就做到了，有多个item我就分配多少内存，做到了memory的节约。并且因为数据结构是通过数组组织的，所以遍历的时候可以用index直接遍历也是很方便的有没有！但是缺点也很明显，查找达不到HashMap O(1)的查找时